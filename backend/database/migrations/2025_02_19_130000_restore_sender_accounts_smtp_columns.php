...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('sender_accounts', function (Blueprint $table) {
            ...
            $table->string('host')->nullable()->after('from_name');
            $table->unsignedSmallInteger('port')->default(587)->after('host');
            $table->string('encryption')->nullable()->after('port'); // null, tls, ssl
    ...
    public function down(): void
    {
        Schema::table('sender_accounts', function (Blueprint $table) {
            $table->dropColumn([
                'from_email', 'from_name', 'host', 'port',
                'encryption', 'password_encrypted', 'username',
            ]);
        });
    }
};
